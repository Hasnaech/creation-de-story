...
// Stories actives (l'API Instagram n'expose les stories que pendant 24 h) :
// vues, portée, réponses, partages et navigation (sorties, sauts) pour
// mesurer la rétention de la séquence en cours.
export async function fetchStoriesPerformance() {
  if (!configOk()) {
    const err = new Error(
      "Connexion Instagram non configurée : renseignez INSTAGRAM_ACCESS_TOKEN et INSTAGRAM_USER_ID dans le fichier .env (voir README)."
    );
  ...
  const stories = await graphGet(`${process.env.INSTAGRAM_USER_ID}/stories`, {
    fields: "id,media_type,timestamp,caption",
  });

  const resultats = [];
  for (const [position, story] of (stories.data || []).entries()) {
    const insights = {};
    // Métriques simples
    try {
      const data = await graphGet(`${story.id}/insights`, { metric: "reach,replies,shares,views" });
      for (const item of data.data || []) insights[item.name] = item.values?.[0]?.value ?? null;
    } catch { /* certaines métriques peuvent être refusées selon le compte */ }
    // Navigation détaillée (sorties, retours, sauts en avant)
    try {
      const nav = await graphGet(`${story.id}/insights`, {
        metric: "navigation",
        breakdown: "story_navigation_action_type",
      });
      const detail = nav.data?.[0]?.total_value?.breakdowns?.[0]?.results || [];
      for (const r of detail) {
        insights[`nav_${r.dimension_values?.[0]}`] = r.value;
      }
    } catch { /* navigation indisponible sur certains comptes */ }

    resultats.push({
      position: position + 1,
      id: story.id,
      date: story.timestamp,
      legende: story.caption || "",
      vues: insights.views ?? null,
      portee: insights.reach ?? null,
      reponses: insights.replies ?? 0,
      partages: insights.shares ?? 0,
      sorties: insights.nav_exit ?? null,
      sautsAvant: insights.nav_tap_forward ?? null,
      retours: insights.nav_tap_back ?? null,
    });
  }
  return resultats;
}
