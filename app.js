// Application Express partagée entre le serveur local (server.js)
// et le déploiement Vercel (api/index.js).
...
const branding = JSON.parse(
  readFileSync(path.join(__dirname, "config", "branding.json"), "utf8")
);
...
// Protection optionnelle : si CODE_ACCES est défini (recommandé quand l'app
// est hébergée en ligne, ex. Vercel), les routes POST exigent ce code. Sans
// lui, n'importe qui trouvant l'URL consommerait les crédits API.
app.use("/api", (req, res, next) => {
  const code = process.env.CODE_ACCES;
  if (!code || req.method === "GET") return next();
  if (req.get("x-code-acces") === code) return next();
  res.status(401).json({ erreur: "Code d'accès requis ou invalide.", codeRequis: true });
});
...
app.get("/api/statut", (_req, res) => {
  res.json({
    ...
    codeRequis: Boolean(process.env.CODE_ACCES),
  });
});
